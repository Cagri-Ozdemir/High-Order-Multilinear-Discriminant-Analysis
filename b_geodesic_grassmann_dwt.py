...
for i in range(num_class):
    ...
    sel = K_fold[3][0]####!!!!!! Select K_fold[0][0] for the first set. K_fold[4][0] for the 5th set.!!!!!!!!!!!!!!!!!!!!!! 
...
for jj in range(len(Test_set)):
  for ii in range(len(Train_set)):
   ###test data
   ut,st,vt = tSVDdwt(Test_set[jj])
   utt = ut[:,:,:eig]
   ###train data
   u,s,v = tSVDdwt(Train_set[ii])
   u0 = u[:,:,:eig]
   uu, ss, vv = tSVDdwt(tproddwt(ttransx(utt),u0))
...
sn.heatmap(cm/20, annot=True,fmt='.1%', cmap='Blues',cbar=False)
